...
exports.pagarConCheckoutPro = async (req, res) => {
  ...
  // Solo validar monto si NO es una llamada de limpieza post-pago
  if (!pagoExitoso && (!monto || isNaN(monto) || Number(monto) <= 0.1)) {
  ...
  try {
    ...
    // PROCESAR VENTA Y LIMPIAR CARRITO SOLO SI pagoExitoso
    if (pagoExitoso && email) {
      try {
        ...
        for (const item of carrito.productos) {
          ...
          if (!detalleProyecto) throw new Error(`No se encontró detalle de proyecto para el producto: ${item.producto_id}`);
          ...
          if (isNaN(stockActual) || isNaN(cantidadVenta) || cantidadVenta <= 0) {
            throw new Error(`Stock o cantidad inválida para el producto: ${item.producto_id}`);
          }
